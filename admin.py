...
def dashboard(c: dict, view: str, channel: str, start: str, end: str, session_id: str = "", range_name: str = "") -> str:
    ...
    if view == "config":
        ...
        limits = {"max_history_messages": int(c.get("max_history_messages", 32)), "context_summary_threshold": int(c.get("context_summary_threshold", 10000)), "max_message_chars": int(c.get("max_message_chars", 5000)), "summary_max_chars": int(c.get("summary_max_chars", 2500))}
        body += f"<div class=panel><h2>消息入口</h2><form method=post action=/config><div class=grid><div><label>当前渠道</label><select name=channel><option value=dingtalk {selected_d}>钉钉</option><option value=wechat {selected_w}>微信</option></select></div><div style='display:flex;align-items:end'><button>保存渠道</button></div></div><div class=grid style='margin-top:18px'><div><label>钉钉 Client ID</label><input name=client_id value='{html.escape(c.get('dingtalk', {}).get('client_id', ''))}'></div><div><label>钉钉 Client Secret</label><input type=password name=client_secret value='{html.escape(c.get('dingtalk', {}).get('client_secret', ''))}'></div></div><div class=grid style='margin-top:18px'><div><label>保留最近消息数</label><input type=number name=max_history_messages min=1 max=200 value='{limits['max_history_messages']}'></div><div><label>自动摘要阈值（字符）</label><input type=number name=context_summary_threshold min=1000 max=100000 value='{limits['context_summary_threshold']}'></div><div><label>单条消息上限（字符）</label><input type=number name=max_message_chars min=500 max=20000 value='{limits['max_message_chars']}'></div><div><label>摘要最大长度（字符）</label><input type=number name=summary_max_chars min=500 max=10000 value='{limits['summary_max_chars']}'></div></div></form></div>" + access_panel
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/config":
            ...
            save_config({"channel": data.get("channel", ["dingtalk"])[0], "dingtalk": {"client_id": data.get("client_id", [""])[0], "client_secret": data.get("client_secret", [""])[0]}, "admin_password": old.get("admin_password", PASSWORD), "full_access": bool(old.get("full_access", False)), "max_history_messages": bounded("max_history_messages", 32, 1, 200), "context_summary_threshold": bounded("context_summary_threshold", 10000, 1000, 100000), "max_message_chars": bounded("max_message_chars", 5000, 500, 20000), "summary_max_chars": bounded("summary_max_chars", 2500, 500, 10000)})
